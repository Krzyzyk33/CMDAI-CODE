...
def print_chat_history(context):
    ...
    tool_queue = []
    for msg in getattr(context, 'full_messages', context.messages):
        role = msg.get('role')
        ...
        elif role == 'user' and str(content).startswith('<tool_response>'):
            continue
        ...
        if is_tool_res:
            if tool_queue:
                ...
                if name_lower in ['read_file', 'view_file']:
                    ...
                    if 'Exit code: 0' in res_str: print_tool_result('Command successful')
                    else: print_tool_result('Command failed')
                elif name_lower in ['ls', 'glob', 'code_search']:
                    if 'Error' not in res_str:
                        title_suffix = ''
                        if name_lower == 'ls':
                            p = args.get('path', '.')
                            if not p or p == '.' or p == '/' or p == '\\': p = os.path.basename(os.path.abspath('.'))
                            title_suffix = f' {p}'
                    ...
                    if name_lower in ['tests', 'run_tests']:
                        for line in res_str.splitlines(): print_tool_result(line, escape_text=True)
                    else:
                        for line in res_str.splitlines():
                            from rich.markup import escape
                            line_esc = escape(line)
                            if line_esc.strip().startswith('|_'): console.print(f"[bright_black]     {line_esc}[/bright_black]", highlight=False)
                            else: console.print(f"[bright_black]  ⎿  {line_esc}[/bright_black]", highlight=False)
                else:
            ...
            if "[COMPRESSED SESSION CONTEXT]" in content:
                ...
                console.print(f"[{MUTED_COLOR}]  ⎿  [Wczytano skompresowany kontekst historii][/]")
                continue
            ...
            if think_text:
                console.print("\n✻ Thinking...", highlight=False)
                for t_line in think_text.splitlines():
                    if t_line.strip():
                        from rich.markup import escape
                        if t_line.lstrip().startswith("|_"):
                            console.print(f"[bright_black]  {escape(t_line)}[/bright_black]", highlight=False)
                        else:
                            console.print(f"[bright_black]    {escape(t_line)}[/bright_black]", highlight=False)
                ...
                for tc in msg['tool_calls']:
                    ...
                    if name in ["write_file", "create_file"]:
                        ...
                        if "code" in args:
                            ...
                            print_code_panel("Python Run (Script)", args["script_path"], lexer_override="text")
def main():
    console.clear()
