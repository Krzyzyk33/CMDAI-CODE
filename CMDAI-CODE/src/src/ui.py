...
def print_header(model_name: str, cwd: str):
    ...
    WHITE = "#ffffff"
    GRAY = "#888888"
    GAP = "   "  # odstęp między CMDAI i CODE - podmień na "  " lub "    " wg gustu

    # ustal kolory wg 4-dniowego cyklu (dzień roku % 4)
    ...
    try:
        ...
        # przytnij puste linie z końca (ale zachowaj wyrównanie wysokości obu bloków)
    ...
    if has_pyfiglet:
        ...
        full_width = max_cmdai_width + len(GAP) + max((len(l) for l in code_lines), default=0)
        padding = max(0, (term_width - full_width) // 2)

        for cmdai_line, code_line in zip_longest(cmdai_lines, code_lines, fillvalue=""):
            header_text.append(" " * padding)
            header_text.append(cmdai_line.ljust(max_cmdai_width), style=cmdai_color)
            header_text.append(GAP)
            header_text.append(code_line, style=code_color)
            header_text.append("\n")
    else:
        # fallback - stary blokowy ascii, całość jednym kolorem (biały)
        ascii_logo = [
            " ███ ██ ██ ██  ███ ███   ███ ███ ██  ███",
            " █   █ █ █ █ █ █ █  █    █   █ █ █ █ █  ",
            " █   █   █ █ █ ███  █    █   █ █ █ █ ███",
        ...
        for line in ascii_logo:
            ...
            header_text.append(" " * padding + line + "\n", style=WHITE)

    dir_short = cwd.replace("\\", "/").rstrip("/")
    sub = f"{dir_short}  ·   {model_name}  ·   {engine_short}"
    sub_padding = max(0, (term_width - len(sub)) // 2)
    header_text.append(" " * sub_padding + sub, style="#666666")

    panel = Panel(
        header_text, border_style="#999999", padding=(0, 1), box=box.ROUNDED
    )
    console.print(panel)
...
def print_tool_call(tool_name: str, arg_summary: str, source_id: str = None):
    ...
    display = TOOL_DISPLAY.get(tool_name.lower())
    if display:
        label = display["label"]
        if arg_summary and display.get("summary_arg"):
            text = f"{label}: {arg_summary}"
        else:
            text = label
        _cprint(f"\n[bold]● {escape(text)}[/bold]", source_id=source_id)
    else:
        _cprint(f"\n[bold]● Tool ({tool_name}): {escape(arg_summary)}[/bold]", source_id=source_id)
    import sys
    sys.stdout.flush()
...
def print_answer_summary(res_str: str, source_id: str = None):
    import re
    from rich.markup import escape

    lines = res_str.splitlines() if isinstance(res_str, str) else []
    if not lines:
        return

    _cprint("[bright_black]    |_ Summary of answers:[/bright_black]", source_id=source_id, highlight=False)
    for line in lines:
        l = line.rstrip()
        if not l or l.startswith("Summary of answers:"):
            continue
        if re.match(r"^\s*\|_\s*\d+\.", l):
            m = re.match(r"^\s*\|_\s*(\d+\..*)", l)
            if m:
                _cprint(f"[bright_black]        |_ {escape(m.group(1))}[/bright_black]", source_id=source_id, highlight=False)
            else:
                _cprint(f"[bright_black]        {escape(l)}[/bright_black]", source_id=source_id, highlight=False)
        elif "➔" in l:
            m = re.search(r"➔\s*(.*)", l)
            ans_text = m.group(1) if m else l
            _cprint(f"[bright_black]             |_ ➔ {escape(ans_text)}[/bright_black]", source_id=source_id, highlight=False)
        elif "↳" in l:
            m = re.search(r"↳\s*(.*)", l)
            desc_text = m.group(1) if m else l
            _cprint(f"[bright_black]                  ↳ {escape(desc_text)}[/bright_black]", source_id=source_id, highlight=False)
        else:
            _cprint(f"[bright_black]        {escape(l.strip())}[/bright_black]", source_id=source_id, highlight=False)
    import sys
    sys.stdout.flush()
...
class ThinkingTree:
    ...
    def render(self):
        ...
        frame_idx = int(elapsed_sec * 3) % len(self.spinner_frames)
        sym = self.spinner_frames[frame_idx]
        ...
        info_str = f"({format_time(elapsed_sec)}{model_str})"

        if self.title == "Subagents":
            t.append(
                f"{prefix}{self.tree_prefix}● {self.title}\n", style=ACCENT_COLOR
            )
        else:
            t.append(
                f"{prefix}{self.tree_prefix}{sym} {self.title}... {info_str}\n",
                style=ACCENT_COLOR,
            )
        if self.expanded:
    ...
    def print_tree(self):
        ...
        prefix = "\n"
        if self.source_id:
            info_str = f"({self.model_name})" if self.model_name else f"({self.source_id})"
        else:
            model_str = f" · {self.model_name}" if self.model_name else ""
            info_str = f"({format_time(elapsed_sec)}{model_str})"
        ...
        if self.title == "Subagents":
            ...
            _cprint(
                f"{prefix}[{ACCENT_COLOR}]{self.tree_prefix}✻ {self.title}... {info_str}[/]",
                source_id=self.source_id,
            )
        if self.expanded:
            pass  # removed empty line
