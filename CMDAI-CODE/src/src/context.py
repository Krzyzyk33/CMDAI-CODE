...
15. [LEAD AGENT DUTY]: When wakeup_subagents finishes, as the Lead Agent you MUST inspect subagent reports and then execute the necessary tool calls yourself to verify the subagent's work and continue the task. Do not treat a subagent report as finished work without independent verification.
16. [SUBAGENT THINKING LEVEL]: When calling wakeup_subagents to create subagents, you MUST assign an appropriate thinking_level ('low', 'medium', 'high', 'ultra', 'extreme') to each subagent based on task complexity.
...
class ContextManager:
    ...
    def get_token_count(self, include_tool_defs: bool = True) -> int:
        ...
        try:
            ...
            for m in self.messages:
                if m.get("hidden") or m.get("role") == "subagent":
                    continue
                if m.get("content"):
            ...
            count += len(enc.encode(str(self.system_prompt or "")))
            if include_tool_defs:
                from .tools import TOOLS_DEFINITIONS
                tools_str = json.dumps(TOOLS_DEFINITIONS, ensure_ascii=False)
                count += len(enc.encode(tools_str))
        except ImportError:
            import json
            for m in self.messages:
                if m.get("hidden") or m.get("role") == "subagent":
            ...
            count += int(len(str(self.system_prompt or "")) // 3.5)
            if include_tool_defs:
                from .tools import TOOLS_DEFINITIONS
                tools_str = json.dumps(TOOLS_DEFINITIONS, ensure_ascii=False)
                count += int(len(tools_str) // 3.5)
        return int(count)
    ...
    def trigger_compaction(self, model):
        ...
        # Keep full message history intact on disk and in memory
        self.save_history()
    ...
    def get_messages(self, tools_desc: str = "", thinking_desc: str = "") -> list:
        ...
        raw_msgs = [m for m in self.messages if not m.get("hidden") and m.get("role") != "subagent"]
