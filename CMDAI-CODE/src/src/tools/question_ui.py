...
def render_question_content(state: AskUserState) -> list[tuple[str, str]]:
    ...
    q_lines = textwrap.wrap(q.question, width=85)
    if not q_lines:
        q_lines = [q.question]
    for idx, ql in enumerate(q_lines):
        prefix = "  " if idx == 0 else "  "
        lines.append(("class:question.text", f"{prefix}{ql}\n"))
        
    if q.description:
        d_lines = textwrap.wrap(q.description, width=83)
        if not d_lines:
            d_lines = [q.description]
        for idx, dl in enumerate(d_lines):
            prefix = "  ↳ " if idx == 0 else "    "
            lines.append(("class:question.desc", f"{prefix}{dl}\n"))
        lines.append(("", "\n"))
    else:
        lines.append(("", "\n"))
    ...
    for i, opt in enumerate(q.options):
        ...
        if opt.description:
            opt_d_lines = textwrap.wrap(opt.description, width=81)
            if not opt_d_lines:
                opt_d_lines = [opt.description]
            for idx, dl in enumerate(opt_d_lines):
                prefix = "    ↳ " if idx == 0 else "      "
                lines.append(("class:option.desc", f"{prefix}{dl}\n"))
            lines.append(("", "\n"))
        else:
            lines.append(("", "\n"))
