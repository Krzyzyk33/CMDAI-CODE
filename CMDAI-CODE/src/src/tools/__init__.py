...
TOOLS_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Wake up one or more subagents to run tasks. ALWAYS specify 'thinking_level' ('low', 'medium', 'high', 'ultra', 'extreme') for each subagent based on task complexity! ALWAYS use native JSON function calling! EXAMPLE CALL: {\"subagents\": [{\"name\": \"CodeArchitect\", \"task\": \"Design 3D game implementation plan\", \"thinking_level\": \"high\"}]}",
            "parameters": {
                "type": "object",
                "properties": {
                    "subagents": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "thinking_level": {
                                    "type": "string",
                                    "description": "Requested thinking level ('low', 'medium', 'high', 'ultra', 'extreme'). ALWAYS specify this field based on task complexity.",
                                },
                                "context_files": {
                                    "type": "array",
                                    "items": {"type": "string"},
                            ...
                            "required": ["name", "task", "thinking_level"],
                        },
                    }
                },
                "required": ["subagents"],
...
TOOL_DISPLAY = {
    "read_file":        {"label": "Read",        "summary_arg": "path"},
    "view_file":        {"label": "Read",        "summary_arg": "path"},
    "create_file":      {"label": "New file",    "summary_arg": "path"},
    "edit_file":        {"label": "Edit",        "summary_arg": "path"},
    "write_file":       {"label": "Overwrite",   "summary_arg": "path"},
    "replace_lines":    {"label": "Edit",        "summary_arg": "path"},
    "append_file":      {"label": "Edit",        "summary_arg": "path"},
    "delete_file":      {"label": "Delete",      "summary_arg": "path"},
    "commands":         {"label": "Command",     "summary_arg": "command"},
    "run_bash":         {"label": "Command",     "summary_arg": "command"},
    "run_tests":        {"label": "Tests",       "summary_arg": None},
    "tests":            {"label": "Tests",       "summary_arg": None},
    "grep":             {"label": "Code Search", "summary_arg": "pattern"},
    "run_grep":         {"label": "Code Search", "summary_arg": "pattern"},
    "code_search":      {"label": "Code Search", "summary_arg": "query"},
    "glob":             {"label": "Glob",        "summary_arg": "pattern"},
    "ls":               {"label": "Ls",          "summary_arg": "path"},
    "list_dir":         {"label": "Ls",          "summary_arg": "path"},
    "search_web":       {"label": "Web Search",  "summary_arg": "query"},
    "web":              {"label": "Web Search",  "summary_arg": "query"},
    "save_plan":        {"label": "Plan",        "summary_arg": None},
    "todo_done":        {"label": "Todo done",   "summary_arg": "step_number"},
    "submit_plan":      {"label": "Plan",        "summary_arg": None},
    "bugs":             {"label": "Bugs",        "summary_arg": None},
    "run_bugs":         {"label": "Bugs",        "summary_arg": None},
    "wakeup_subagents": {"label": "Subagents",   "summary_arg": None},
    "answer":           {"label": "Answer",      "summary_arg": None},
    "ask_question":     {"label": "Answer",      "summary_arg": None},
    "python":           {"label": "Python Script", "summary_arg": "command"},
    "run_python":       {"label": "Python Script", "summary_arg": "command"},
    "read":             {"label": "Read",        "summary_arg": "path"},
    "edit":             {"label": "Edit",        "summary_arg": "path"},
    "create":           {"label": "New file",    "summary_arg": "path"},
    "write":            {"label": "Overwrite",   "summary_arg": "path"},
}


def get_arg_summary(name: str, args: dict) -> str:
    display = TOOL_DISPLAY.get(name.lower(), {"label": name, "summary_arg": None})
    arg_key = display["summary_arg"]
...
RESULT_DISPLAY = {
    "read_file":        {"format": "Read {lines} lines",                          "needs": ["lines"]},
    "view_file":        {"format": "Read {lines} lines",                          "needs": ["lines"]},
    "edit_file":        {"format": "Edited {path} (+{added}/-{removed})",        "needs": ["path", "added", "removed"]},
    "replace_lines":    {"format": "Edited {path} (+{added}/-{removed})",        "needs": ["path", "added", "removed"]},
    "append_file":      {"format": "Appended to {path}",                          "needs": ["path"]},
    "write_file":       {"format": "Created/Overwritten {path} ({lines} lines)", "needs": ["path", "lines"]},
    "create_file":      {"format": "Created/Overwritten {path} ({lines} lines)", "needs": ["path", "lines"]},
    "delete_file":      {"format": "Deleted {path}",                              "needs": ["path"]},
    "commands":         {"format": None},  
    "run_bash":         {"format": None},  
    "run_tests":        {"format": None},  
    "tests":            {"format": None},  
    "grep":             {"format": None},  
    "code_search":      {"format": None},  
    "glob":             {"format": None},  
    "ls":               {"format": None},  
    "list_dir":         {"format": None},  
    "bugs":             {"format": None},  
    "run_bugs":         {"format": None},  
    "wakeup_subagents": {"format": None},  
    "save_plan":        {"format": "Plan saved",                                  "needs": []},
    "submit_plan":      {"format": "Plan submitted",                              "needs": []},
    "todo_done":        {"format": "Step {step} done",                            "needs": ["step"]},
    "search_web":       {"format": None},  
    "answer":           {"format": None},
    "ask_question":     {"format": None},
}


def get_tool_result_summary(name: str, args: dict, result: str) -> Optional[str]:
    cfg = RESULT_DISPLAY.get(name.lower())
    if cfg is None:
        return None
    fmt = cfg["format"]
    if fmt is None:
    ...
    for key in needs:
        if key == "lines":
            vals["lines"] = len(result.splitlines())
        elif key == "added":
            vals["added"] = len(args.get("new_str", "").splitlines()) if "new_str" in args else 0
        elif key == "removed":
            vals["removed"] = len(args.get("old_str", "").splitlines()) if "old_str" in args else 0
        elif key == "step":
            vals["step"] = args.get("step_number", "?")
        elif key in args:
            vals[key] = args[key]
        elif key == "path":
            vals["path"] = args.get("path", "file")
        else:
            vals[key] = "?"
    return fmt.format(**vals) if vals else fmt
